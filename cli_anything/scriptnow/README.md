...
- **新增卷章 = 纯追加（服务端硬门禁）**：`storymap append-volume` / `append-chapters` / `append-phase` 只尾部新增，已有卷章完全不动；纯追加形状的全量提案会被服务端拒绝并指引追加通道；旧结构自动归档，`novel/script storymap-restore` 可导出恢复候选。
