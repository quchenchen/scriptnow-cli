...
6. 实例由宿主 Agent 托管时（`SCRIPTNOW_HOSTED=1`），登录会话通常已由宿主在服务端换发并写入
   `SCRIPTNOW_CLI_CONFIG`：**不要运行 `scriptnow login`**（它要等一个只在用户自己电脑上
   可达的浏览器回调，在这里必然超时）。确实需要重新授权时，用 **`scriptnow login --device`**：
   CLI 给出确认码与链接，用户在自己已登录的浏览器里确认，凭据不经过命令行。
   **不要向用户索取或要求粘贴 Cookie / 密码**，也不要自行调用换发端点或读取 CLI 会话文件。
   提示未登录时，先请宿主重新下发会话，或稍后重试。
...
（大作品数分钟起），不要把它当默认入口、也不要在里面干等。
没有作者批准的来源画像，改编项目不能生成任何规划候选 —— 这条门禁不因来源来自本地而放松。
