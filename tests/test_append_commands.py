...
def test_login_rejects_password_inputs(monkeypatch):
    from click.testing import CliRunner
    import cli_anything.scriptnow.utils.browser_login as browser
    from unittest.mock import Mock
    called = Mock(return_value=Mock(base_url="https://x.test"))
    monkeypatch.setattr(browser, "browser_login", called)
    runner = CliRunner()
    for args in (["login", "--password", "secret"], ["login", "--password-stdin"], ["--password", "secret", "project", "list"]):
        result = runner.invoke(main, args)
        assert result.exit_code != 0
        assert "secret" not in result.output
    result = runner.invoke(main, ["login", "--host", "https://x.test", "--json"], env={"SCRIPTNOW_PASSWORD": "secret"})
    assert result.exit_code == 0
    assert "secret" not in result.output
