...
- **Append-only structure growth (server-side shape gate)**: `storymap append-volume` / `append-chapters` / `append-phase` add volumes/chapters
  without touching existing ones; full-channel proposals in pure-append shape (including any-position additions) are rejected with
  append-channel guidance; **full replacement (retained=0) is also rejected for ordinary full-channel proposals** — restoring an old
  structure is only legal via `novel/script storymap-restore` (server validates the archive mirror), and brand-new structures are
  limited to first creation (empty structure) or the storymap-rebuild-* isolated chain; replaced structures are archived and reviewable.
...
script mirror `script storymap-archives <pid>` / `script storymap-archive <pid> <archive_id>`; incident rollback: `novel storymap-restore <pid> <archive_id>` / `script storymap-restore <pid> <archive_id>` export a restore candidate (overwrite = restructure; full review chain + confirm). — both carry
