...
| 组 | 用途 |
|----|------|
| guide | 聚焦式新手创作向导（固定 12 步顺序：登录→创建→方向→故事核心与蓝图→梗概→粗纲→StoryMap 与集纲/章纲一体→Skill→正文→审读→导出→完成）：`--step 1..12 --medium novel|script`；`--pulse/--resume` 柔性回归；`--steps` 查看全图；`--complete/--status` 完成标记与完成状态 |
...
长篇剧本粗纲先统筹全剧，再按项目叙事结构分宏观阶段，阶段内根据项目自身集数与批次策略深化。
“约 5 集”只是默认建议，不是固定集数或强制边界；Agent 必须读取 `generation_batches`，不得自行猜测。
...
- **编排前置：Skill 是逐章/逐场创作前的必然门禁（MANDATORY，且须健壮性完善）**：
  ...
  （可多轮），再试写样本章节/场次检验 Skill 约束力、诊断缺口并迭代加固
  （interpret local 蒸馏 或 skill craft 六问），然后在平台创建并挂载到项目，最后
  `skill mounts <pid>` 核实已挂载，才能启动正文逐章/逐场创作。`book` 也会在缺 Skill 时硬停提示。
